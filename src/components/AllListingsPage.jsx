...
import { MapPin, Clock, Heart, Star, ArrowLeft } from 'lucide-react';
import { motion } from 'framer-motion';
import { Button } from '@/components/ui/button';
import { supabase } from '@/lib/customSupabaseClient';

const AllListingsPage = ({ onListingClick, currentUser, onLikeToggle, onBack }) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST200') {
           ...
           if (fallbackData) {
             const formatted = fallbackData.map(l => ({...l, userName: 'Utilisateur', userAvatarUrl: null}));
    ...
    <div className="min-h-screen bg-muted py-8">
      <div className="container mx-auto px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <h1 className="text-4xl font-bold text-foreground mb-2">Toutes les annonces</h1>
          <p className="text-muted-foreground text-lg">Explorez toutes les offres disponibles sur AFRMARKET</p>
        </motion.div>
        ...
        <div className="mb-8 max-w-2xl mx-auto">
          <input
            ...
            className="w-full px-6 py-4 bg-card border-2 border-input rounded-full focus:border-orange-500 focus:outline-none shadow-sm"
          />
        </div>

        {loading ? (
          <div className="text-center py-20">
            <p className="text-muted-foreground text-xl">Chargement des annonces...</p>
          </div>
        ) : (
          <>
